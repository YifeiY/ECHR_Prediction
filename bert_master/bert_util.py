'''Author: Yifei
For Fine Tuning BERT and mini-batch-backprop-single-batch training'''
...
class BERT():
  ...
  def __init__(self, bert_config_file, vocab_file, init_checkpoint, batch_size=4):
  ...
  def encode_case(self,case):
    ...
    '''return BERT representation given an ECHR case in
    Shelf :: {'headings': String, 'body': [String]}.'''

    # Input

    examples = read_examples(case)
    ...
    self.input_fn = input_fn_builder(features=features, seq_length=self.max_seq_length)
    ...
    document_features = []
    for result in self.estimator.predict(self.input_fn, yield_single_examples=True):
      unique_id = int(result["unique_id"])
      feature = unique_id_to_feature[unique_id]
      facts_features = [token for (_, token) in enumerate(feature.tokens)]
      document_features.append(facts_features)
...
def main():
  bert = BERT(bert_config_file="bert_master/pre-trained/uncased_L-12_H-768_A-12/bert_config.json",
              vocab_file="bert_master/pre-trained/uncased_L-12_H-768_A-12/vocab.txt",
              init_checkpoint="bert_master/pre-trained/uncased_L-12_H-768_A-12/bert_model.ckpt",
              batch_size=4)


  doc_adapter = DocumentAdapter()
  doc_adapter.load_shelf(shelf='train',partition='train')
